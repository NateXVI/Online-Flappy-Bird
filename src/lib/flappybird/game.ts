...
export const game = {
	...
	update(p: p5, deltaSec: number) {
		...
		if (p.millis() - game.startTime < 1200) {
			bird.pos.x = p.lerp(-121, p.width / 4, (p.millis() - game.startTime) / 1000);
		} else {
	...
	updatePipes: (p: p5) => {
		...
		if (p.abs(p.width + 70 - game.pipes[game.pipes.length - 1].pos.x) >= pipeSpacing) {
	...
	getWindowSize: (p: p5) => {
		...
		const height = p.constrain(p.windowHeight, minWindowHeight, maxWindowHeight);
