...
export const drawStaticBackground = (p: p5) => {
	...
	p.image(assets.image.backgroundImage, backgroundOffset + p.width, 0, p.width, p.height);
	p.pop();
};
...
export const drawGround = (p: p5, birdAlive = false, deltaSec: number) => {
	...
	for (let i = 0; i < 7; i++) {
		p.image(assets.image.groundImage, i * w + groundOffset - i, p.height - 90, w, h);
	}
	p.pop();
...
export const drawTitleScreen = (p: p5, deltaSec: number) => {
	...
	titleTextScale = p.constrain(titleTextScale + titleTextDuration * deltaSec, 0, 1);
	p.pop();
...
export const drawScoreScreen = (p: p5) => {
	...
	p.text('PRESS SPACE TO RESTART', scoreScreen.x, p.height / 2 + scoreScreen.scale * 42 * uiScale);
