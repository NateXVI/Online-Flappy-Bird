class Pipe {
	constructor() {
		this.pos = createVector(
			width + 80,
			random(height / 4, height - height / 3)
		);
		this.scale = 0.6;
	...
	draw() {
		...
		image(
			game.pipeSpriteDown,
			this.pos.x,
			this.pos.y - this.height / 2 - this.gap / 2 - this.height + 80,
			this.width,
			this.height
		);
		image(
...
function updatePipes() {
	if (
		abs(width + 70 - game.pipes[game.pipes.length - 1].pos.x) >= pipeSpacing
	) {
		game.pipes.push(new Pipe());
		if (game.pipes[game.pipes.length - 2].pos.y) {
			let pos2 = game.pipes[game.pipes.length - 2].pos.y;
			game.pipes[game.pipes.length - 1].pos.y = constrain(
				game.pipes[game.pipes.length - 1].pos.y,
				pos2 - tolerance,
				pos2 + tolerance
			);
		}
