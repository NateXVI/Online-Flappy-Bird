...
function setup() {
	...
	try {
		let m = localStorage.getItem('muted') == 'true' ? true : false;
		muteGame(m);
		console.log(m);
	} catch (error) {
		// console.log(error);
...
function drawBackground() {
	push();
	image(backgroundImage, backgroundOffset, 0, width, height);
	image(backgroundImage, backgroundOffset + width, 0, width, height);
...
game.updateGame = function () {
	for (let i = 0; i < game.pipes.length; i++) {
		if (
			game.pipes[i].pos.x < game.bird.pos.x &&
			game.pipes[i].previousPos >= game.bird.pos.x
		) {
			game.score += 1;
...
function updateScoreScreen() {
	if (millis() - game.endTime < scoreScreen.animationDuration) {
		scoreScreen.pos.x = lerp(
			-scorePanel.width * scoreScreen.scale,
			width / 2,
			(millis() - game.endTime) / scoreScreen.animationDuration
		);
	} else {
...
function muteGame(isMuted = null) {
