const leaderboard = {
	get: async function () {
		...
		let response = await fetch(
			'https://flappy-bird-leaderboard.herokuapp.com/api/get/leaderboard',
			{ method: 'get' }
		);
	...
	saveScore: async function () {
		...
		if (name && name != '') {
			...
			let response = await fetch(
				`https://flappy-bird-leaderboard.herokuapp.com/api/post/score?player_id=${id}&player_name=${name}&player_score=${score}`
			);
		}
	},
};
...
document.querySelector('button.save-button').addEventListener('click', () => {
